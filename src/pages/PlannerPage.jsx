...
// No longer need: import { getFunctions, httpsCallable } from 'firebase/functions';
...
const PlannerPage = () => {
  ...
  const handleAddPlan = async (e) => {
    ...
    try {
      // --- CHANGE START ---
      const response = await fetch('/api/process-plan', { // Call your Vercel Function endpoint
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-user-id': user.uid // Pass user ID as a custom header
        },
        body: JSON.stringify({ planInput: planInput })
      });

      const result = await response.json(); // Parse the JSON response from your Vercel Function

      if (response.ok && result.status === 'success') { // Check if HTTP status is OK and custom status is success
        alert('Plan processed by AI and saved!');
        setPlanInput('');
      } else {
        setSubmitError(result.message || 'Failed to process plan via AI.');
      }
      // --- CHANGE END ---
...
// ... (plannerStyles object remains the same)
const plannerStyles = {
    container: {
      padding: '30px',
